...
def setup_counting():
    ...
    number_vehicle = 0
    #while cap.isOpened():
def counting_vehicle(green,number_frame,ret_val,frame,width,height,net,list_object, skip_frame=1):
    number_vehicle =1
    #number_frame += 1
    # Read frame
    #ret_val, frame = cap.read()
    ...
    if green:
                ...
                if not check_location(box_y, box_height, height):
                    ...
                    if check_new_object and check_start_line(box_y, box_height):
                        ...
                        if frame is None:
                            ...
                            directory = r'E:\NAMHOC\python\Pytorch\data'
                            os.chdir(directory)
                            filename = 'save.jpg'
                            mask = frame[int(box_y-box_y/4):int(box_y+box_height),int(box_x-box_x/4):int(box_x+box_width)]
        ...
        #            (0, 0, 255), 2)
        # Draw start line
        cv2.line(frame, (0, START_POINT), (width, START_POINT), (255, 255, 255), 4)
        # Draw end line
    ...
    return frame
#detect = False
#counting_vehicle(detect,r'E:\NAMHOC\python\DO_AN\lala.mp4', 'vehicles.avi')
